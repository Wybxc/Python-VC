...
def compute_f0(wav, sr=16000, frame_period=10.0):
    ...
    f0, _ = pyworld.harvest(  # type: ignore
...
def get_converted_lf0uv(
    ...
    lf0_vc = lf0_src.copy()
    lf0_vc[lf0_src > 0.0] = (
        lf0_src[lf0_src > 0.0] - lf0_mean_src
    ) / lf0_std_src * lf0_std_trg + lf0_mean_trg
    f0_vc = lf0_vc.copy()
...
def convert_continuos_f0(f0):
    ...
    uv: np.ndarray = np.float32(f0 != 0)  # type: ignore
...
def get_cont_lf0(f0, frame_period=10.0, lpf=False):
    ...
    if lpf:
        cont_f0_lpf = low_pass_filter(
            cont_f0, int(1.0 / (frame_period * 0.001)), cutoff=20
        )
