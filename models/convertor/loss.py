...
class MaskedMSELoss(nn.Module):
    ...
    def get_mask(self, lengths, max_len=None):
        ...
        seq_range_expand = (
            seq_range.unsqueeze(0).expand(batch_size, max_len).to(lengths.device)
        )
        seq_length_expand = lengths.unsqueeze(1).expand_as(seq_range_expand)
        return (seq_range_expand < seq_length_expand).float()

    def forward(
        self, mel_pred, mel_pred_postnet, mel_trg, lengths, stop_target, stop_pred
    ):
        ## process stop_target
        ...
        stop_mask = self.get_mask(
            stop_lengths, int(mel_trg.size(1) / self.frames_per_step)
        )
        ...
        mel_loss_pre = (
            self.mel_loss_criterion(mel_pred, mel_trg) * mel_mask
        ).sum() / mel_mask.sum()
        mel_loss_post = (
            self.mel_loss_criterion(mel_pred_postnet, mel_trg) * mel_mask
        ).sum() / mel_mask.sum()
        ...
        # stop token loss
        stop_loss = (
            torch.sum(self.stop_loss_criterion(stop_pred, stop_target) * stop_mask)
            / stop_mask.sum()
        )
