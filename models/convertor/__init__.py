...
class MelDecoderMOLv2(AbsMelDecoder):
    ...
    def forward(
        ...
        spk_embeds = (
            F.normalize(spembs).unsqueeze(1).expand(-1, decoder_inputs.size(1), -1)
        )
        ...
        # (B, num_mels, T_dec)
        T_dec = torch.div(
            feature_lengths, int(self.encoder_down_factor), rounding_mode="floor"
        )
        ...
        if output_att_ws:
            return self.parse_output(
                [mel_outputs, mel_outputs_postnet, predicted_stop, alignments],
                speech_lengths,
            )
        else:
            return self.parse_output(
    ...
    def inference(
        ...
        if bottle_neck_features.size(0) > 1:
            mel_outputs, alignments = self.decoder.inference_batched(
