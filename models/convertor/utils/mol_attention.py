...
class MOLAttention(nn.Module):
    ...
    def initialize_bias(self):
        ...
        if self.r == 2:
            torch.nn.init.constant_(
                self.query_layer[2].bias[2 * self.M : 3 * self.M], 1.8545
            )
        elif self.r == 4:
            torch.nn.init.constant_(
                self.query_layer[2].bias[2 * self.M : 3 * self.M], 3.9815
            )
        elif self.r == 1:
            torch.nn.init.constant_(
                self.query_layer[2].bias[2 * self.M : 3 * self.M], 0.5413
            )
        else:
            torch.nn.init.constant_(
                self.query_layer[2].bias[2 * self.M : 3 * self.M], -0.432
            )
    ...
    def init_states(self, memory):
        ...
        self.J = (
            torch.arange(0, T_enc + 2.0).to(device) + 0.5
        )  # NOTE: for discretize usage
    ...
    def forward(self, att_rnn_h, memory, memory_pitch=None, mask=None):
        ...
        phi_t = w.unsqueeze(-1) * (
            1 / (1 + torch.sigmoid((mu_cur.unsqueeze(-1) - j) / sigma.unsqueeze(-1)))
        )
        # print("phi_t:", phi_t)
