...
class Decoder(nn.Module):
    ...
    def __init__(
        ...
        # Attention RNN
        self.attention_rnn = nn.LSTMCell(prenet_dims[-1] + enc_dim, attention_rnn_dim)
        ...
        for i in range(num_decoder_rnn_layer):
            if i == 0:
                self.decoder_rnn_layers.append(
                    nn.LSTMCell(enc_dim + attention_rnn_dim, decoder_rnn_dim)
                )
            else:
                self.decoder_rnn_layers.append(
                    nn.LSTMCell(decoder_rnn_dim, decoder_rnn_dim)
                )
        ...
        if concat_context_to_last:
            self.linear_projection = Linear(
                enc_dim + decoder_rnn_dim, num_mels * frames_per_step
            )
        else:
            self.linear_projection = Linear(decoder_rnn_dim, num_mels * frames_per_step)
    ...
    def initialize_decoder_states(self, memory, mask):
        ...
        self.attention_hidden = torch.zeros((B, self.attention_rnn_dim), device=device)
        self.attention_cell = torch.zeros((B, self.attention_rnn_dim), device=device)
        ...
        for i in range(self.num_decoder_rnn_layer):
            self.decoder_hiddens.append(
                torch.zeros((B, self.decoder_rnn_dim), device=device)
            )
        ...
        self.attention_context = torch.zeros((B, self.enc_dim), device=device)
    ...
    def parse_decoder_inputs(self, decoder_inputs):
        ...
        decoder_inputs = decoder_inputs.reshape(
            ...
            int(decoder_inputs.size(1) / self.frames_per_step),
            -1,
        )
    ...
    def decode(self, decoder_input):
        for i in range(self.num_decoder_rnn_layer):
            if i == 0:
                (
                    self.decoder_hiddens[i],
                    self.decoder_cells[i],
                ) = self.decoder_rnn_layers[i](
                ...
                (
                    ...
                    self.decoder_hiddens[i - 1],
                    (self.decoder_hiddens[i], self.decoder_cells[i]),
                )
    ...
    def forward(self, memory, mel_inputs, memory_lengths):
        ...
        while len(mel_outputs) < decoder_inputs.size(0) - 1:
            ...
            if self.concat_context_to_last:
                decoder_rnn_output = torch.cat((decoder_rnn_output, context), dim=1)
    ...
    def inference(self, memory, stop_threshold=0.5):
        ...
        # NOTE(sx): heuristic
        max_decoder_step = (
            memory.size(1) * self.encoder_down_factor // self.frames_per_step
        )
        min_decoder_step = (
            memory.size(1) * self.encoder_down_factor // self.frames_per_step - 5
        )
        while True:
            ...
            if (
                torch.sigmoid(stop_output.data) > stop_threshold
    ...
    def inference_batched(self, memory, stop_threshold=0.5):
        ...
        while True:
            ...
            if (
                torch.all(torch.sigmoid(stop_output.squeeze().data) > stop_threshold)
                and len(mel_outputs) >= min_decoder_step
            ):
                break
            if len(mel_outputs) >= max_decoder_step:
        ...
        for mel, stop_logit in zip(mel_outputs, stop_outputs):
            idx = np.argwhere(torch.sigmoid(stop_logit.cpu()) > stop_threshold)[0][
                0
            ].item()
