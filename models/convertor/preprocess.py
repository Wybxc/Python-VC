...
import models.embed.inference as Encoder
from models.embed import audio
...
def _compute_spkEmbed(wav, output_fpath, encoder_model_local, device):
    ...
    wave_slices, mel_slices = Encoder.compute_partial_slices(
        len(wav), rate=1.3, min_pad_coverage=0.75
    )
...
def preprocess_one(wav_path, out_dir, device, ppg_model_local, encoder_model_local):
    ...
    _, length_bnf = _compute_bnf(
        output_fpath=f"{out_dir}/bnf/{utt_id}.ling_feat.npy",
        wav=wav,
        device=device,
        ppg_model_local=ppg_model_local,
    )
    _, length_f0 = _compute_f0_from_wav(
        output_fpath=f"{out_dir}/f0/{utt_id}.f0.npy", wav=wav
    )
    _, length_embed = _compute_spkEmbed(
        output_fpath=f"{out_dir}/embed/{utt_id}.npy",
        device=device,
        encoder_model_local=encoder_model_local,
        wav=wav,
    )
